...
colorize ()
{
    ...
    if [ "$2" = "f" ]; then
        ...
        # switch statement
        case "$FILETYPE" in
          bz|bz2|ear|gz|hqx|img|iso|jar|rpm|sar|sit|srpm|tar|tgz|war|zip|rar)
          ...
          BIN|EXE|GROOVY|PL|SH|SQL)
            echo -ne "\033[0;32m" #green
            ;;
          gif|img|jpg|jpeg|png|ps|psd|swf|tif|tiff|raw)
            echo -ne "\033[0;35m" #purple
            ;;
          GIF|IMG|JPG|JPEG|PNG|PS|PSD|SWF|TIF|TIFF|RAW)
            echo -ne "\033[0;35m" #purple
            ;;
          avi|mov|mpeg|mpg|mp3|wav|ogg|flac|wmv|mkv|mp4|m2v|ts)
            echo -ne "\033[0;33m" #orange
            ;;
          AVI|MOV|MPEG|MPG|MP3|WAV|OGG|FLAC|WMV|MKV|MP4|M2V|TS)
            echo -ne "\033[0;33m" #orange
            ;;
          doc|pdf|ppt|rtf|xls|docx|pptx|txt|xls|xlsx|epub|cbz|cbr)
...
function walktree ()
{
    ...
    if [ ${2:-x} == "true" -o ${2:-x} == "x" ]; then
        ...
        FILEPATHS=`find ${1-.} -print | egrep -v '.DS_Store$|.localized$|CVS'`
    else
        # sort find
